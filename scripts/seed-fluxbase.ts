...
import { escapeSql, generateId } from '../src/lib/fluxbase'
import { createFluxbaseTables } from '../src/lib/fluxbase-schema'

const API_KEY = process.env.FLUXBASE_API_KEY;
const BASE_URL = process.env.FLUXBASE_BASE_URL;
const PROJECT_ID = process.env.FLUXBASE_PROJECT_ID;
const SCHEMA = 'project_' + PROJECT_ID;

async function rawExec(sql: string) {
  const r = await fetch(BASE_URL + '/api/execute-sql', {
    method: 'POST',
    headers: {
...
async function main() {
  console.log('Seeding Fluxbase database...')

  // Step 1: Create tables
  console.log('Creating tables...')
  await createFluxbaseTables()

  // Step 2: Insert skills
  console.log('Creating skills...')
  const skillMap = new Map<string, string>()
  for (const s of skills) {
    const sql = `INSERT INTO skill (id, name, category, description, created_at) VALUES ('${generateId()}', '${escapeSql(s.name)}', '${escapeSql(s.category)}', '${escapeSql(s.description)}', CURRENT_TIMESTAMP) ON CONFLICT (name) DO UPDATE SET category = EXCLUDED.category, description = EXCLUDED.description RETURNING *`
    const rows = await rawExec(sql)
    if (rows.length > 0) {
      skillMap.set(s.name, rows[0].id as string)
    }
  }
  ...
  for (const [skillName, prereqName, importance] of prerequisites) {
    ...
    try {
      const sql = `INSERT INTO skill_prerequisite (id, skill_id, prerequisite_skill_id, importance) VALUES ('${generateId()}', '${escapeSql(skillId)}', '${escapeSql(prereqId)}', '${escapeSql(importance)}') ON CONFLICT (skill_id, prerequisite_skill_id) DO UPDATE SET importance = EXCLUDED.importance`
      await rawExec(sql)
      prereqCount++
    } catch (e) {
  ...
  // Step 4: Insert role requirements
  ...
  for (const [role, skillName, level, importance] of roleRequirements) {
    ...
    try {
      const sql = `INSERT INTO role_skill_requirement (id, target_role, skill_id, required_level, importance) VALUES ('${generateId()}', '${escapeSql(role)}', '${escapeSql(skillId)}', '${escapeSql(level)}', '${escapeSql(importance)}') ON CONFLICT (target_role, skill_id) DO UPDATE SET required_level = EXCLUDED.required_level, importance = EXCLUDED.importance`
      await rawExec(sql)
      roleCount++
    } catch (e) {
  ...
  // Step 5: Insert resources + resource skills
  console.log('Creating resources...')
  let resourceCount = 0
  for (const r of resources) {
    try {
      const resSql = `INSERT INTO resource (id, title, description, url, type, difficulty, estimated_hours, quality_score, created_at) VALUES ('${generateId()}', '${escapeSql(r.title)}', '${escapeSql(r.description)}', '${escapeSql(r.url)}', '${escapeSql(r.type)}', '${escapeSql(r.difficulty)}', ${r.estimatedHours}, ${r.qualityScore}, CURRENT_TIMESTAMP) RETURNING *`
      const resRows = await rawExec(resSql)
      if (resRows.length === 0) continue
      const resourceId = resRows[0].id as string
      resourceCount++

      for (const skillName of r.skills) {
        const skillId = skillMap.get(skillName)
        if (!skillId) continue
        const rsSql = `INSERT INTO resource_skill (id, resource_id, skill_id) VALUES ('${generateId()}', '${escapeSql(resourceId)}', '${escapeSql(skillId)}') ON CONFLICT (resource_id, skill_id) DO NOTHING`
        await rawExec(rsSql)
      }
    } catch (e) {
  ...
  console.log('Fluxbase seed complete!')
}
